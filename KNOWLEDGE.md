...
| MOC.md | 主题索引 | tag 维度交叉索引 | 每次 Ingest（脚本生成）| 按时代/地区/技法等维度浏览 |
...
3. **全量实体提取**：逐页逐行逐字阅读源全文。每遇到一个人、地、物、窑口、著作、机构、事件——只要是一个独立存在的东西——立即建 entities/ 页面。不判断「值不值得建」「信息够不够」——先建。信息少就是 stub，信息多就写详。**原子写入 + 行号标注**，见上方 🔴 规则。
