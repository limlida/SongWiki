...
python3 .agents/skills/openclaw-kb-selfcheck/scripts/selfcheck.py
...
tags 按维度前缀打标：`时代/` `地区/` `材料/` `技法/` `窑口/` `制度/` `器物/` `人物/`。同概念归一化，粒度不混杂。详见 `KNOWLEDGE.md` §编译时 LLM 标记。
...
- 标 `aliases`（同义词，防重复建页）
- 标 `tags`（按维度分层）
- 发现幽灵实体（源中有但 wiki 未建页）→ 记录在任务文件批次记录中
...
- 暗概念 → 保留为规则 #13
- 幽灵实体 → 移到编译时（LLM 在 Ingest 时记录到任务文件批次记录）
...
- `tags`：按维度分层打标（时代 / 地区 / 材料 / 技法 / 窑口 / 制度 / 器物 / 人物），index.md 维度扫描依赖
